...
var theater = {
	...
	volume: 25,
	syncMaxDiff: 10,
	forceFlashOnYoutube: true, //!(navigator.userAgent.indexOf("Awesomium")==-1),
...
//	if ( navigator.userAgent.indexOf("Awesomium")==-1 || !theater.forceFlashOnYoutube ) {
	if ( !theater.forceFlashOnYoutube ) {
		/*
			Embed Player Object
		*/
		var player;
		...
		*/
		...
		this.setVideo = function( id ) {
			...
			this.player.setup({
				...
				primary: 'flash',
				displaytitle: true,
				file: "https://youtube.com/watch?v=" + id
			});
		...
		*/
		...
		this.think = function() {
			...
			*/
				...
				if ( this.player.getPlaylist()[0] && !this.sentAltInfo && this.player.getState() === "playing" && this.player.getDuration() > 0 ) {
	...
	var TheaterRTMP = function() {
		...
		var viewer = videojs('player', {techOrder: ['flash']});
	//	viewer.poster("https://winterphoenix96.github.io/rtmp-thumbnails/default.png");
		...
		*/
		this.think = function() {
			...
			if ( this.player != null ) {
				if ( this.videoId != this.lastVideoId ) {
					this.player.src({ type: "rtmp/flv", src: "rtmp://98.194.102.151:1935/live&"+this.videoId});
					this.lastVideoId = this.videoId;
	...
	registerPlayer( "rtmp", TheaterRTMP );
	var JWPlayer_URL = function() {
		// JW7 Key
		jwplayer.key="GBbtI9R8M4R2gQOTSs7m7AdoMdxpK3DD4IcgmQ==";
	...
	registerPlayer( "http", JWPlayer_URL );
	registerPlayer( "https", JWPlayer_URL );
	registerPlayer( "twistmoe", JWPlayer_URL );
	registerPlayer( "kissanimeru", JWPlayer_URL );
	registerPlayer( "raw_url", JWPlayer_URL );
